...
export default function Index({ title, width, image, iconBgColor, borderColorClass, link, soon, offline }: TileProp) {
    ...
    return <Link to={soon ? "#" : link} className="relative">
        <section style={{ width: width }} className={`flex relative  active:bg-green-200 transition-all ${!isOnline && `border border-red-200`} bg-[#f9f9f9] items-center gap-2 py-2 px-2 rounded-full`}>
            ...
            <section className={`flex-center w-[40px] h-[40px] basis-[60px] rounded-full ${iconBgColor} border ${borderColorClass}`}>
                {image}
            </section>
            <span className="text-sm text-slate-700  basis-[60px]">{title}</span>
            {offline && <div className="bg-offline-400 h-[65px] absolute w-full text-center left-0 bottom-[-20px] pt-[48px] text-white text-xs z-[-10] rounded-full">آفلاین</div>}
        </section>
