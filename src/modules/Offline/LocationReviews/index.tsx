...
export default function Index() {
    ...
    const { list, syncData, actionLoading, clearOfflineReviews } = useLocationReviews()
    return <main className="pb-[80px]">
        ...
        {list.map((item: OfflineReview, index: number) => (<Card clearOfflineReviews={clearOfflineReviews} key={index} id={item.id} expireDate={item.expireDate} isError={item.isError} item={item.locateReviews} />))}
