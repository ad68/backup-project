...
export default function Index() {
    const { list, syncData } = useLocationReviews()
    return <main className="pb-[80px]">
        <PageTitle size="small" title="لیست تعیین مکان آفلاین" />
        <div className="px-2 flex justify-center">
            <CustomButton disabled={list.length === 0} className="bg-purple-600 w-full" onClick={() => syncData()}>
                همگام سازی به حالت آنلاین
                <UploadCloudIcon />
            </CustomButton>
        </div>
        {list.map((item: OfflineReview, index: number) => (<Card key={index} id={item.id} item={item.locateReviews} />))}
