...
export default function Index() {
    const { isAddDocumentModal, setIsAddDocumentModal, takePhotoModalIsOpen, setTakePhotoModalIsOpen, actionLoading, data, deleteFile, getFileList } = useDocument()
    return <>
        <section className="p-2">
            <section className="border border-slate-200  rounded-lg p-2">
                ...
                {/* {!actionLoading && <section className="grid grid-cols-2 gap-2">
                    {data.map((item: any, index: number) => (<FileItem deleteFile={deleteFile} key={index} item={item} />))}
                </section>}
