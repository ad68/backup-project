...
export default function Index({ isOpen, setIsOpen, setSearchParams }: FilterModalProp) {
    const { provinces, setProvinceId, counties, setCountyId, searchList, provinceId, countyId, setPolicyId, clearForm } = useFilter(setSearchParams)
    return <SlidingModal isOpen={isOpen} keepChildren={true}>
        <section className="p-4">
            ...
            <section className="mt-2">
                ...
                <CustomTextBox onChange={(e) => { setPolicyId(e) }} placeholder="شناسه ذینفع را وارد کنید" />
            </section>
            ...
            <section className="mt-1">
                <span className="font-light text-slate-700 text-xs">شماره بازدید:</span>
                <CustomSelect value={countyId} options={counties.map((item: any) => ({ label: item.title, value: item.id }))} onChange={(e) => { setCountyId(e) }} />
            </section>
